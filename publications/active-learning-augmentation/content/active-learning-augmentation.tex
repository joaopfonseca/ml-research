...
% TODO: Discuss the benefits of Data Augmentation in AL
...
implement Geometric SMOTE as a data augmentation method beyond the typical
...
performance metrics. We found that the usage of a heuristic data augmentation
method in an AL implementation almost always improves its performance,
regardless of the AL implementation's goal.
...
Augmentation algorithm is sufficient to significantly increase the data
selection efficiency in AL\@. The usage of data augmentation reduced the
amount of iterations required to train a classifier with a performance as good
as (or better than) classifiers trained with the entire training dataset
(\textit{i.e.,} without using AL). The proposed method was capable of reducing
the training dataset and use the most informative observations for the
training phase of the classifier, which is expanded with artificial data. 
...
The further improvement of the proposed method will require future work to (1)
focus on the development of methods with varying data augmentation policies
depending on the different input space regions, (2) develop
...
better understand the gap between heuristic/input space data augmentation techniques
and neural network/feature space data augmentation techniques in an AL context.
