...
two different categories~\cite{Kumar2020}: 
...
Research on more robust selection criteria has been developed in order to
address the efficiency problem. This is motivated by the importance of the
selection criteria in AL's iterative process~\cite{Rosario2020}. Specifically,
Settles~\cite{Settles2011} observed that informed selection criteria fails to
outperform the random selection of observations in some datasets. Generally,
the Random Selection method is used as a baseline method. This method
disregards the class membership probabilities produced by the classifier and
returns N random points from the dataset without following any specific
criteria.
...
\subsection{Modifications to the AL framework}

Modifications to the AL framework have been developed in order to address the
efficiency problem.

Learning loss in AL~\cite{Yoo2019}
...
Discuss the importance of domains to determine the appropriate data
augmentation methods.

\subsection{Heuristic Data Augmentation}
...
\subsection{Neural Network-based Data Augmentation}
