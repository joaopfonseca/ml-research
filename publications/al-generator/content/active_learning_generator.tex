...
\begin{figure}[htb]
    ...
    \caption{
        Proposed AL framework. \added{This paper's contribution, }the data
        generation mechanism, is represented as the generator (marked
        with~\textit{C}), which is used to add artificial instances to the
        data generation phase. \deleted{The remaining steps are left
        unchanged.}
    }~\label{fig:al_new}
\end{figure}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=.51\linewidth]{../analysis/scenes}
...
Figure~\ref{fig:dur}. A DUR above 1 means that the proposed framework requires
...
to the amount of data required by the baseline framework).
