...
development of more efficient \replaced{classifiers }{choosers} and selection
...
Resampling solutions modify the \added{training} data\deleted{set} by removing
...
\replaced{Since we are interested in the introduction of artificial data
generation in AL, we will analyze the state-of-the-art on resampling
techniques (specifically oversampling) in Section~\ref{sec:ovs-sota}}{ Because
of this we will focus on artificial data generation techniques, presented in
...
additional component in the iterative AL procedure (the generator) \added{that
...
of iterations required until \replaced{the classifier reaches a satisfactory
performance }{convergence of the classifier's quality}.
...
\begin{enumerate}
    \item Unlabeled dataset. Consists of the original data source (or a sample
        ...
        criterion to expand the training \added{data}set in regions where the
        classification uncertainty is higher. Therefore, the \added{unlabeled}
        dataset is used for both producing the initial training
        \added{dataset} by selecting a set of \added{instances} for the
        supervisor to annotate (discussed in point 3) and calculating the
        uncertainty map to augment the training dataset.
    \item Supervisor. \replaced{A human annotator (or team of human
        annotators)}{An external entity} to which the uncertainty map is
        ...
        Cawley2011}.
    \item Initial training dataset. It is a small\added{, labeled} sample of
        \added{the original} data \added{source} used to initiate the first AL
        iteration. The size of the initial training sample normally varies
        between no instances at all and 10\% \added{of the unlabeled
        dataset}~\cite{Li2013}.
    \item Current and expanded training dataset. It is the concatenation of
        the initial training \added{dataset} and the datasets labeled by the
...
prediction output \replaced{can be}{ is} identified as the LULC map. This
...
lack of a robust selection criterion and/or \replaced{classifier}{chooser} may
...
\begin{figure}[htb]
    ...
    \caption{
        Proposed AL framework. \added{This paper's contribution, comprises a
        change in the AL framework through the introduction of a data
        generation mechanism, } \deleted{the data generation mechanism, is}
        represented as the generator (marked with~\textit{C}), which is used
        to add artificial instances to the \replaced{training dataset. }{data
        generation phase.} \deleted{The remaining steps are left unchanged.}
    }~\label{fig:al_new}
\end{figure}
...
The performance metric \textit{Geometric mean} (G-mean) \added{and
\textit{F-score} are} less sensitive to the data imbalance
bias~\cite{Jeni2013, Kubat1997}.  Therefore, we employ \added{both of these
...
\begin{table}[H]
    ...
    \caption{
        ...
        of G-SMOTE \added{almost} always improves the results of the original
        framework.
...
variable used for this test is the data utilization rate \added{based on the
...
baseline AL framework \deleted{(NONE)}, \added{with the exception of one
comparison,} as shown in Table~\ref{tab:aulc_ranks} \added{and
Figure~\ref{fig:dur}}. \replaced{This means the proposed}{The} AL
\added{framework} using data generation was able to outperform the baseline AL
in \added{nearly} all scenarios. 
