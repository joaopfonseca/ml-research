...
However, Data Augmentation methods may be used for different
purposes other than regularization. For example, Vector Quantised Generative
Adversarial Networks (VQGAN)~\cite{Esser2020} is a Deep Learning approach that
...
(CLIP)~\cite{Radford2021}. Figure~\ref{fig:super_miles_dali} shows an example of an image
generated using the VQGAN+CLIP model. Style transferring techniques have also been
used to augment image datasets and successfully improved classification
performance~\cite{Wang2017}.
...
observations closest to the decision boundary.
...
Benefits of heuristic methods in contexts of limited data availability.

Small data augmentation.

In~\cite{Fonseca2021active} the authors apply data augmentation 

Risk of overfitting in small data augmentation.

Other heuristic augmentation strategies.
...
Non-linear combination of input observations
