...
class Datasets:
    ...
    def download(self):
        ...
        if self.names == 'all':
            func_names = [
                func_name for func_name in dir(self) if 'fetch_' in func_name
            ]
        else:
            func_names = [
                f'fetch_{name}'.lower().replace(' ', '_')
...
class ImbalancedBinaryDatasets(Datasets):
    ...
    def fetch_ecoli(self):
        ...
        """
        data = pd.read_csv(
            FETCH_URLS['ecoli'], header=None, delim_whitespace=True
        )
    ...
    def fetch_heart(self):
        ...
        """
        data = pd.read_csv(
            FETCH_URLS['heart'], header=None, delim_whitespace=True
        )
    ...
    def fetch_vehicle(self):
        ...
        for letter in ascii_lowercase[0:9]:
            ...
            partial_data['target'] = partial_data['target']\
                .isin(['van']).astype(int)
...
class BinaryDatasets(Datasets):
    ...
    def fetch_breast_cancer(self):
        ...
        data = pd.concat([
            data.drop(columns=[0, 1]),
            data[[1]].rename(columns={1: 'target'})],
            axis=1
        )
...
class ContinuousCategoricalDatasets(Datasets):
    ...
    def fetch_covertype(self):
        ...
        wilderness_area = pd.Series(
            np.argmax(data.iloc[:, 10:14].values, axis=1),
            name=10
        )
        soil_type = pd.Series(
            np.argmax(data.iloc[:, 14:54].values, axis=1),
            name=11
        )
        data = data\
    ...
    def fetch_flags(self):
        ...
        data = data.drop(columns=[0, 6])\
            .T.reset_index(drop=True).T.join(target)
        categorical_features = [
            0, 1, 4, 8, 9, 10, 11, 12, 13, 14, 15, 21, 22, 23, 24, 25, 26, 27
        ]
        return data, categorical_features
...
class RemoteSensingDatasets(Datasets):
    ...
    def fetch_kennedy_space_center(self):
        ...
        """
        df = img_array_to_pandas(
            *self._load_gic_dataset('kennedy_space_center')
        )
        return df[df.target != 0]
